...
function FailureCard({ item, principal, reload }: { item: FailureItem; principal: DemoPrincipal; reload: () => Promise<void> }): ReactNode {
  ...
  async function retry(): Promise<void> {
    ...
    try {
      ...
      await reload();
      setMessage("Processing attempt recorded.");
    } catch (reason) {
  ...
  return <article className="failure-card"><div className="item-topline"><h3>{item.synthetic_reference}</h3><span className={item.resolved ? "resolved-badge" : "failure-badge"}>{item.resolved ? "Resolved" : "Current failure"}</span></div><details><summary>Processing details and history</summary><dl className="failure-meta"><dt>Failed stage</dt><dd>{item.failed_stage}</dd><dt>Diagnostic code</dt><dd>{item.diagnostic_code}</dd><dt>First attempt</dt><dd>{new Date(item.first_attempt_at).toLocaleString()}</dd><dt>Latest attempt</dt><dd>{new Date(item.latest_attempt_at).toLocaleString()}</dd><dt>Attempts</dt><dd>{item.attempt_count}</dd><dt>Terminal state</dt><dd>{item.current_terminal_state}</dd></dl><ol className="attempt-list">{item.attempt_history.map((attempt) => <li key={attempt.attempt_id}><b>Attempt {attempt.attempt_number}</b><span>{attempt.state}</span>{attempt.diagnostic_code && <small>{attempt.diagnostic_code}</small>}</li>)}</ol></details>{!item.resolved && <button type="button" className="secondary-button" disabled={working || !item.retryable} onClick={() => void retry()}>{item.retryable ? "Retry synthetic processing" : "Permanent failure · Retry unavailable"}</button>}<p className="form-message" aria-live="polite">{message}</p></article>;
}
...
export function FailurePage({ principal }: { principal: DemoPrincipal }): ReactNode {
  ...
  return <><section className="page-title"><h1>Synthetic failure queue</h1><p>Inspect failed calls and retry eligible processing.</p></section><section className="queue-section"><div className="section-heading"><h2>Current failures</h2><span className="count-badge">{queue.current.length}</span></div>{queue.current.length === 0 && <p>No current failures.</p>}{queue.current.map((item) => <FailureCard item={item} principal={principal} reload={load} key={item.call_id} />)}</section><section className="queue-section"><div className="section-heading"><h2>Resolved history</h2><span className="count-badge">{queue.resolved.length}</span></div>{queue.resolved.map((item) => <FailureCard item={item} principal={principal} reload={load} key={item.call_id} />)}</section></>;
}
